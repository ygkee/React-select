...
var Select = _react2['default'].createClass({
	...
	popValue: function popValue() {
		...
		if (valueArray[valueArray.length - 1].disabled != undefined && valueArray[valueArray.length-1].disabled) return;
	...
	clearValue: function clearValue(event) {
		...
		var valueArray = this.getValueArray();
		this.setValue(valueArray.filter(function (value) {
            return value.disabled != undefined && value.disabled;
        }));
		this.setState({
			isOpen: false,
			inputValue: ''
		...
		if (this.props.multi) {
			return valueArray.map(function (value, i) {
				return _react2['default'].createElement(
					ValueComponent,
					{
						disabled: _this2.props.disabled || (value.disabled != undefined && value.disabled),
	...
	filterOptions: function filterOptions(excludeOptions) {
		...
		if (typeof this.props.filterOptions === 'function') {
			...
			if (filterValue) {
			    options.sort(function(o, ob) {
			        return o[_this3.props.labelKey].toLowerCase().indexOf(filterValue) <= ob[_this3.props.labelKey].toLowerCase().indexOf(filterValue) ? -1 : 1;
			    });
	        }
	        return options;
		} else {
	...
	render: function render() {
        ...
        if (this.props.allowCreate && this.state.inputValue.trim() && this.props.newOptionCreator) {
            var equal = false;
            options.concat(valueArray).map(function(option, i) {
                if (option[this.props.labelKey].trim() == this.state.inputValue.trim()) {
                    equal = true;
                }
            }.bind(this));
            if (!equal) {
                var inputValue = this.state.inputValue.trim();
                var newOption = this.props.newOptionCreator(inputValue);
